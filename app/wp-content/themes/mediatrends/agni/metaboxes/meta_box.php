...
define( 'CUSTOM_METABOXES_DIR', get_template_directory_uri() . '/agni/metaboxes' );
	...
	switch( $type ) {
		...
		case 'post_chosen':
			...
			foreach ( $posts as $item )
				...
				foreach( $post_type as $post_type_object ){
		...
		case 'image':
			...
			echo	'<input name="' . esc_attr( $name ) . '" type="hidden" class="meta_box_upload_image" value="' . intval( $meta ) . '" />
						<img src="' . esc_attr( $image ) . '" class="meta_box_preview_image" alt="" />
							...
							<small>&nbsp;<a href="#" class="meta_box_clear_image_button">Remove</a></small></div>
							<br clear="all" />' . $desc;
		break;
		// file
		case 'file':		
