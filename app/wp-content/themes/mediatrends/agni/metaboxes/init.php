...
class cmb_Meta_Box {
	...
	public function register_scripts() {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'crispy' ),
					'defaultString' => __( 'Default', 'crispy' ),
					'pick'          => __( 'Select Color', 'crispy' ),
					'current'       => __( 'Current Color', 'crispy' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[&uarr;]&nbsp;',
			'down_arrow'      => '&nbsp;[&darr;]',
			'check_toggle'    => __( 'Select / Deselect All', 'crispy' ),
		) ) );
	...
	public static function render_group( $args ) {
		...
		echo '<tr><td colspan="2"><table id="', $field_group->id(), '_repeat" class="repeatable-group'. $sortable .'" ">';
		if ( $desc || $label ) {
			echo '<tr><th>';
				if ( $label )
			...
			<td>
				...
				if ( $field_group->options( 'group_title' ) ) {
					...
					<tr class="cmb-group-title">
						<th colspan="2">
							', sprintf( '<h5>%1$s</h5><div class="fancy-line-small"></div>', $field_group->replace_hash( $field_group->options( 'group_title' ) ) ), '
						<th>
					</tr>
					';
				}
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'crispy' ) );
