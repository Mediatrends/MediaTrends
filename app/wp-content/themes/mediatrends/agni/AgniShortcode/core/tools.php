...
class Su_Tools {
	...
	public static function get_carousel( $args ) {
		$args = wp_parse_args( $args, array(
				'source'  => 'none',
				'limit'   => 20,
				'gallery' => null,
		...
		if ( !is_array( $args['source'] ) ) return $slides;
		// Default posts query
		$query = array( 'post_type' => 'portfolio', 'posts_per_page' => $args['limit'] );
		// Source: media
		if ( $args['source']['type'] === 'media' ) {
			$query['post_type'] = 'attachment';
			$query['post_status'] = 'any';
	...
	public static function example() {
		...
		// Cache not found
			...
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="su-examples-code"><span class="su-examples-get-code button"><i class="icon code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'su' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
	...
	public static function reset_examples() {
		foreach ( (array) Cosme_Data::examples() as $example ) foreach ( (array) $example['items'] as $item ) delete_transient( 'su/examples/render/' . $item['id'] );
	}
	...
	public static function icon( $src = 'file' ) {
		return ( strpos( $src, '/' ) !== false ) ? '<img src="' . $src . '" alt="" />' : '<i class="icon ' . $src . '"></i>';
	}
	...
	public static function get_icon( $args ) {
		...
		if ( strpos( $args['icon'], 'icon:' ) !== false ) {
			...
			su_query_asset( 'css', 'ionicons' );
			// Return icon
			return '<i class="icon ' . trim( str_replace( 'icon:', '', $args['icon'] ) ) . '" style="' . $args['style'] . '"></i>';
		}
		// Image icon
	...
	public static function icons() {
		$icons = array();
		if ( is_callable( array( 'Cosme_Data', 'icons' ) ) ) foreach ( (array) Cosme_Data::icons() as $icon ) {
				$icons[] = '<i class="icon ' . $icon . '" title="' . $icon . '"></i>';
			}
